defmodule WalReplication.Handler do
  ...
  @impl true
  def call(message, %PostgresReplication{replication_slot_name: replication_slot_name})
      when is_write(message) do
    %Write{message: message} = parse(message)
    [{pid, _}] = Registry.lookup(Registry.Handler, replication_slot_name)
